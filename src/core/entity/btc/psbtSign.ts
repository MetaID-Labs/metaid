...
export function classicToPsbt(tx: utxoTx, network?: Network, maximumFeeRate?: number): Psbt {
	...
	tx.outputs.forEach((output: utxoOutput) => {
		if (output.omniScript) {
			...
			const outputData: PsbtOutputExtended = {
	...
	for (let i = 0; i < psbt.inputCount; i++) {
		...
		if (isTaprootInput(input)) {
			...
			signer.publicKey = Buffer.from(
				taproot.taprootTweakPubkey(toXOnly(wif2Public(privateKey, network)))[0]
			);
...
export function generateUnsignedBuyingPsbt(
	...
	buyingData.sellerPsbts.forEach((sellerPsbt) => {
		...
		if (!nftUtxo) {
			nftUtxo = Transaction.fromBuffer(psbt.data.inputs[SELLER_INDEX].nonWitnessUtxo!).outs[
				nftInput.index
			];
		}
