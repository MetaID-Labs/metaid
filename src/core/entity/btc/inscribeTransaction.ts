...
export class InscriptionTool {
  ...
  static newInscriptionTool(network: bitcoin.Network, request: InscriptionRequest) {
    ...
    request.metaidDataList.forEach((metaidData) => {
      tool.inscriptionTxCtxDataList.push(
        createInscriptionTxCtxData(network, metaidData, request.commitTxPrevOutputList[0].keyPairs)
      )
    })
  ...
  completeRevealTx() {
    this.revealTxs.forEach((revealTx, i) => {
      ...
      //   schnorr.sign(hash, this.inscriptionTxCtxDataList[i].privateKey, base.randomBytes(32))
      // )
