...
export async function getBiggestUtxo({
  address,
}: {
  address: string;
}): Promise<{
...
export async function getNewBrfcNodeInfo(params: {
  xpub: string;
  parentTxId: string;
}): Promise<{
  address: string;
  ...
  try {
    const data = await axios
      ...
      .then((res) => {
        const node = res.data.result.data.find(
          (item) => item.parentTxId == parentTxId
        );
        return node;
      });
